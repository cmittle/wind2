...
require_once 'config.php'; // Database setting constants [DB_HOST, DB_NAME, DB_USERNAME, DB_PASSWORD]
   require_once '.././libs/jwt_helper.php';
	...
	//If tid is null run query that returns all (first 300 lines) of the gearbox_specifics table
	//$response = array();
	...
	if ($authPass != null) {
		...
		$sql = 'SELECT * FROM  tower_main LIMIT 0 , 1000';
			...
			//if tid is not null then run query that returns only lines (up to 30) with that tid
			$sql = 'SELECT * FROM  tower_main WHERE  tower_id =:tid  LIMIT 0 , 30';
		...
		$stmt = $db->prepare( $sql );
		//this binds the $tid variable to ":tid" so I can use this as a variable directly in query statement written above
