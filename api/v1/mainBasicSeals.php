...
require_once 'config.php'; // Database setting constants [DB_HOST, DB_NAME, DB_USERNAME, DB_PASSWORD]
	...
	try {
	    ...
	    $response['status'] = 'failure in mainBasicSeals.php';
	...
	if ($authPass != null) {
		$response['status'] = "success from mainBasicSeals.php";
        	$response['message'] = 'You are approved';
		if ($tuid == null) {
		$sql = 'SELECT * FROM  main_seal_specifics LIMIT 0 , 300';
		} else {
			//if tuid is not null then run query that returns only lines (up to 30) with that tuid
			$sql = 'SELECT * FROM  main_seal_specifics WHERE  tuid =:tuid  LIMIT 0 , 30';
