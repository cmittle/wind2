app.controller('towerTextCtrl', function ($scope, $uibModal, $filter, $http, $routeParams, $window, $sce) {
    ...
    $scope.towerText;
    $scope.twerTextVersion; //pull version from database results, if does not exist start with 1
    ...
    //should add if statement before to see if towerText is to long before proceeding.
	    if ($scope.towerText.length <= 3000) {
	    	...
	    	$http({
	                ...
	                params: {gbid: $scope.urlModel, text: $scope.gtowerText, version: $scope.towerTextVersion}
	            }).then(function (data) {
